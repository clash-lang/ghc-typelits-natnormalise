...
data SolverContext
  = SolverContext
    ...
    , emittedKnownNat :: IORef (Set CType)
    }

mkSolverContext :: TcPluginM Init SolverContext
mkSolverContext = do
  tcs <- lookupTyCons
  emittedKnownNat <- liftIO $ newIORef (Set.empty :: Set CType)
  return $
    SolverContext
      { tyCons = tcs
      , emittedKnownNat
...
mkKnWanted emittedKnownNat loc ctTy@(CType ty) = do
  shouldEmit <- liftIO $
    atomicModifyIORef' emittedKnownNat $ \seen ->
      if Set.member ctTy seen
      then (seen, False)
      else (Set.insert ctTy seen, True)
