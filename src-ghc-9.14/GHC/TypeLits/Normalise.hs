...
import GHC.Core.Predicate
  (EqRel (NomEq), Pred (EqPred, IrredPred), classifyPredType, mkClassPred,
   mkNomEqPred, isEqClassPred, isEqPred, getClassPredTys_maybe)
...
import GHC.Tc.Types.Constraint
  ( Ct, CtEvidence (..), TcEvDest (..), WantedCtEvidence(..),
   ctEvidence, ctEvCoercion, ctLoc, isGiven,
...
reduceGivens opts leqT done givens =
  let nonEqs =
        ...
        , let ev = ctEvidence ct
              prd = ctEvPred ev
        , isGiven ev
        , not $ (\p -> isEqClassPred p || isEqPred p || isEqPredClass p) prd
        ]
  in filter
      (\(_, (prd, _, _)) ->
...
unifyItemToPredType ui = mkNomEqPred ty1 ty2
  where
    ty1 = case ui of
